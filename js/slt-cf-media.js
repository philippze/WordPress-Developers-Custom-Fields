...
jQuery( document ).ready( function( $ ) {
	...
	$( document.body ).on( 'click.sltcfOpenMediaManager', '.slt-cf-fs-button', function( e ) {
		...
		var allow_multiple_field = button.siblings('.slt-cf-allow-multiple');
		...
		if ( typeof slt_cf_media[ 'allow_multiple__' + field_name ] != 'undefined' ) {
			allow_multiple = slt_cf_media[ 'allow_multiple__' + field_name ];
		}
		...
		slt_cf_media_frames[ field_name ].on( 'select', function() {
			var preview_div, preview_size, i;

			// Grab our attachment selection and construct a JSON representation of the model.
			var media_attachments = [];
			var media_attachment_ids = [];

			var media_frames = slt_cf_media_frames[ field_name ].state().get( 'selection' );

			for ( i = 0; i < media_frames.length; i++) {
				// Send the attachment ID to our custom input field via jQuery.
				media_attachment_ids.push(media_frames.models[i].toJSON().id );
			}

			file_id_field.val( media_attachment_ids.join(','));
			...
			for ( i = 0; i < media_frames.length; i++ ) {
				$.ajax({
					type: 'POST',
					url: slt_cf_media.ajaxurl,
					data: {
						id: 	media_attachment_ids[i],
						size:	preview_size,
						action:	'slt_cf_fs_get_file'
					},
					success: function ( response ) {
						preview_div.append(response);
					}
				});
			}
